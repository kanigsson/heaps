...
package body Heaps.Min_Max_Tournament with SPARK_Mode is
          ...
          Post => Node_Valid (H, Node)
                  ...
                  and (for all I in H.Winners'Range =>
                         (if I /= Node then
                            ...
                            and H.Max_Winners (I) = H.Max_Winners'Old (I)
                            and H.Max_Keys (I) = H.Max_Keys'Old (I)));
   ...
   procedure Recompute (H : in out Heap; Node : Index) is
      ...
      MLP   : constant Extended_Index := Max_Position_At (H, Left);
      MRP   : constant Extended_Index := Max_Position_At (H, Right);
   begin
      if LP = 0 then
         if RP = 0 then
            H.Winners (Node) := 0;
      ...
      if MLP = 0 then
         if MRP = 0 then
            H.Max_Winners (Node) := 0;
         else
            H.Max_Winners (Node) := MRP;
            H.Max_Keys (Node) := Max_Key_At (H, Right);
         end if;
      elsif MRP = 0
        or else Max_Key_At (H, Left) >= Max_Key_At (H, Right)
      then
         H.Max_Winners (Node) := MLP;
         H.Max_Keys (Node) := Max_Key_At (H, Left);
      else
         H.Max_Winners (Node) := MRP;
         H.Max_Keys (Node) := Max_Key_At (H, Right);
      end if;
                  ...
                  and then
                    (for all I in After.Winners'Range =>
                       (if I /= Changed then
                          ...
                          and After.Max_Winners (I) = Before.Max_Winners (I)
                          and After.Max_Keys (I) = Before.Max_Keys (I))),
   ...
   procedure Lemma_Node_Unchanged
     ...
     with Ghost,
          Pre  => Before.Capacity = After.Capacity
                  ...
                  and then Max_Position_At (After, Node)
                             = Max_Position_At (Before, Node)
                  and then Max_Position_At (After, 2 * Node)
                             = Max_Position_At (Before, 2 * Node)
                  and then Max_Position_At (After, 2 * Node + 1)
                             = Max_Position_At (Before, 2 * Node + 1)
                  and then
                    (if Max_Position_At (After, Node) > 0 then
                       Max_Key_At (After, Node) = Max_Key_At (Before, Node))
                  and then
                    (if Max_Position_At (After, 2 * Node) > 0 then
                       Max_Key_At (After, 2 * Node)
                         = Max_Key_At (Before, 2 * Node))
                  and then
                    (if Max_Position_At (After, 2 * Node + 1) > 0 then
                       Max_Key_At (After, 2 * Node + 1)
                         = Max_Key_At (Before, 2 * Node + 1)),
   ...
   procedure Lemma_Leaf_Changed
     ...
     with Ghost,
          Pre  => Is_Heap (Before)
                  ...
                  and then Before.Max_Winners = After.Max_Winners
                  and then Before.Max_Keys = After.Max_Keys
                  and then
                    (for all Node in After.Capacity .. 2 * After.Capacity - 1 =>
                       (if Node /= Leaf then
                          ...
                          and then
                            (if Max_Position_At (After, Node) > 0 then
                               Max_Key_At (After, Node)
                                 = Max_Key_At (Before, Node)))),
   ...
   procedure Lemma_Max_Parent_Beats_Child
     (H : Heap; Parent : Index; Child : Tree_Index)
     with Ghost,
          Pre  => Parent < H.Capacity
                  and then Child in 2 * Parent .. 2 * Parent + 1
                  and then Max_Position_At (H, Child) > 0
                  and then Node_Valid (H, Parent),
          Post => Max_Position_At (H, Parent) > 0
                  and then Max_Key_At (H, Parent) >= Max_Key_At (H, Child);
   ...
   begin
      for I in 1 .. H.Last loop
         ...
         while Node > 1 loop
            Lemma_Heap_Node (H, Node / 2);
            Lemma_Max_Parent_Beats_Child (H, Node / 2, Node);
            Node := Node / 2;

            pragma Loop_Invariant (Node <= 2 * H.Capacity - 1);
            pragma Loop_Invariant (Max_Position_At (H, Node) > 0);
            pragma Loop_Invariant (Max_Key_At (H, Node) >= H.Keys (I));
            pragma Loop_Variant (Decreases => Node);
         end loop;

         pragma Loop_Invariant
           (for all J in 1 .. I =>
              (if H.Capacity = 1 then H.Keys (1) else H.Max_Keys (1))
                >= H.Keys (J));
      end loop;
   end Lemma_Root_Is_Maximum;

   procedure Lemma_Max_Root_Represents_Key (H : Heap)
     with Ghost,
          Pre  => not Is_Empty (H) and then Is_Heap (H),
          Post => Max_Root_Position (H) in 1 .. H.Last
                  and then
                    (if H.Capacity = 1
                     then H.Keys (1)
                     else H.Max_Keys (1))
                      = H.Keys (Max_Root_Position (H));

   procedure Lemma_Max_Root_Represents_Key (H : Heap) is
      Node : Tree_Index := 1;

      procedure Lemma_Winner_Comes_From_Child (Parent : Index)
        with Ghost,
             Pre  => Parent < H.Capacity
                     and then Max_Position_At (H, Parent) > 0
                     and then Node_Valid (H, Parent),
             Post =>
               (Max_Position_At (H, 2 * Parent)
                  = Max_Position_At (H, Parent)
                and then Max_Position_At (H, 2 * Parent) > 0
                and then Max_Key_At (H, 2 * Parent)
                  = Max_Key_At (H, Parent))
               or else
               (Max_Position_At (H, 2 * Parent + 1)
                  = Max_Position_At (H, Parent)
                and then Max_Position_At (H, 2 * Parent + 1) > 0
                and then Max_Key_At (H, 2 * Parent + 1)
                  = Max_Key_At (H, Parent));
   ...
   begin
      ...
      while Node < H.Capacity loop
         declare
            ...
            Right : constant Tree_Index := Left + 1;
         begin
            pragma Assert (Max_Position_At (H, Node) > 0);
            Lemma_Heap_Node (H, Node);
            Lemma_Winner_Comes_From_Child (Node);
            if Max_Position_At (H, Left) = Max_Position_At (H, Node)
              and then Max_Position_At (H, Left) > 0
              and then Max_Key_At (H, Left) = Max_Key_At (H, Node)
            then
               Node := Left;
            else
               pragma Assert
                 (Max_Position_At (H, Right) = Max_Position_At (H, Node));
               pragma Assert (Max_Position_At (H, Right) > 0);
               pragma Assert
                 (Max_Key_At (H, Right) = Max_Key_At (H, Node));
               Node := Right;
            end if;
         end;
         ...
         pragma Loop_Invariant
           (Node <= 2 * H.Capacity - 1);
         pragma Loop_Invariant
           (Max_Position_At (H, Node) = H.Max_Winners (1));
         pragma Loop_Invariant (Max_Position_At (H, Node) > 0);
         pragma Loop_Invariant
           (Max_Key_At (H, Node) = H.Max_Keys (1));
         pragma Loop_Variant (Increases => Node);
      end loop;

      pragma Assert
        (Max_Position_At (H, Node) = Node - H.Capacity + 1);
      pragma Assert
        (Max_Key_At (H, Node) = H.Keys (Max_Root_Position (H)));
   end Lemma_Max_Root_Represents_Key;
   ...
   begin
      Lemma_Root_Is_Maximum (H);
      Lemma_Max_Root_Represents_Key (H);
      return (if H.Capacity = 1 then H.Keys (1) else H.Max_Keys (1));
   end Peek_Max;
   ...
   begin
      Lemma_Root_Is_Maximum (H);
      Lemma_Max_Root_Represents_Key (H);
      Slot := Index (Max_Root_Position (H));
      K := Peek_Max (H);
